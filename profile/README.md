...
| **Large Project**         | [online quiz](https://github.com/aide-examples/aide-quiz)     |
|:--------------------------|:--------------------------------------------------------------|
...
| **Platform Builder:**     | [rap](https://github.com/aide-examples/aide-rap) | 
...
### [AIDE - rap](https://github.com/aide-examples/aide-rap) (rapid application platform)
This is a generic databse platfrom builder (based on aide-frame) which is driven by meta data defined in a set of markdown docs. It is the dream of what "CASE" tools in the 1990s wanted to be. You describe your class model in semi-formal notation and interactively design a graphical layout for the diagram. At the same time SQL tables are generated and a generic three layer backend and a modern browser UI are generated. There is also a generic type system available which allows you to define enums, numeric ranges, regexp patterns for attributes of entities. Validation is identically performed in the backend for integrity reasons and in the front end for ergonomic reasons. Sample data can be imported or generated based on textual notation. In fact AIDE-rap generates prompts for an arbitrary AI system to generate useful sample data that will respect foreign key relationships. The platform builder focuses on data handling (including updates with optimistic concurremcy control, audit trail and many more professional features. It offers an integration API for business process modelling tools like HCS Leap or Microsoft Power Platform.
Aide - rap comes with three small examples but it can be used to build much, much larger systens. A system with 1000 function points will not be a problem. Contact the author to learn more about a medium scale real world example.

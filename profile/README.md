...
| **Platform Builder:**     | [rap](https://github.com/aide-examples/aide-rap) (*planned*) | 
...
This is a **planned** generic platform builder (based on aide-frame) which will be driven by meta data defined in a set of markdown docs. You describe your class model in semi-formal notation and interactively design a graphical layout for the diagram. At the same time SQL tables are generated and a generic three layer backend and a modern browser UI are generated. There will also be a generic type system which allows to define enums, numeric ranges, regexp patterns for attributes of entities. Validation shall be identically performed in the backend for integrity reasons and in the front end for ergonomic reasons. It shall be possible to import sample data or generate data sets based on textual notation. The platform will focus on data handling, including updates with optimistic concurremcy control, audit trail and many more professional features. It will offer an integration API for business process modelling tools like HCS Leap or Microsoft Power Platform.
